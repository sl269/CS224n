...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # dimensions are (src_len, b, h*2), ((2, b, h), (2, b, h))
        enc_X, (last_hidden, last_cell) = self.encoder(packed_X) 
        enc_hiddens = pad_packed_sequence(enc_X, batch_first=False)[0]
        # change dim to (b, src_len, h*2)
        enc_hiddens = enc_hiddens.permute(1, 0, 2) 

        #3
        init_decoder_hidden = self.h_projection(torch.cat(tuple(last_hidden), dim=1)) #h_0^{dec}
        init_decoder_cell = self.c_projection(torch.cat(tuple(last_cell), dim=1)) #c_0^{dec}
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        #1 applying W_{attProj} to h^enc
        enc_hiddens_proj = self.att_projection(enc_hiddens) # (b, src_len, h)
        #2
        Y = self.model_embeddings.target(target_padded) # (tgt_len, b, e)
        #3
        for Y_t in torch.split(Y, 1, dim=0): #split tgt_len so you can iterate over t (word)
            Y_t = torch.squeeze(Y_t, dim=0) #squeeze to (b,e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # (b, src_len, h) , (b, h, 1) <- unsqueezed to this dim..multiplied to give (b, src_len, 1) then squeezes to (b, src_len)
